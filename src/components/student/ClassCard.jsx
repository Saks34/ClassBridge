...
import { Clock, GraduationCap, Play, Video, ArrowRight, Zap } from 'lucide-react';
...
export default function ClassCard({ classData }) {
    ...
    const getButtonLabel = () => {
        if (status === 'Cancelled') return { text: 'Class Cancelled', icon: null };
        if (status === 'Live')      return { text: 'Join Live', icon: Zap };
        if (status === 'Completed') return { text: 'View Recording', icon: Video };
        return { text: 'Preview Class', icon: ArrowRight };
    };
    ...
    return (
        ...
        >
            ...
            {/* Live ping indicator (top-right corner) */}
            {status === 'Live' && (
            ...
            <div className="relative flex flex-col flex-1 p-6 gap-5">
                ...
                <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                        ...
                        >
                            <GraduationCap size={20} style={{ color: accent.from }} />
                        </div>
                        {/* Title */}
                ...
                >
                    {ButtonIcon && (
                        <ButtonIcon
                            size={13}
                            className={status === 'Live' ? 'animate-pulse' : 'transition-transform duration-200 group-hover/btn:translate-x-1'}
                        />
                    )}
